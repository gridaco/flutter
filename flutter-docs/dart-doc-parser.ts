...
function makeTsConstructor(
  name: string,
  params: Array<Parameter<any>>
): string {
  ...
  allocating = JSON.stringify(allocating, null, 4)
    .split('"')
    .join()
    .split(":")
    .join()
...
${JSON.stringify(parametering, null, 4)
  .split('"')
  .join()
  .split(",")
  .join()
...
async function parseDocs(docs: string, type: docType) {
  switch (type) {
    ...
    case "constructor":
      ...
      const parameterTypes = xpath
        .select(xp_parameterTypes, doc)
        .map((n) => n.data);
      const parameterNames = xpath
        .select(xp_parameterNames, doc)
        .map((n) => n.data);
